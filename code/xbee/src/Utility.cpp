...
namespace XBEE {
    ...
    template <typename T> std::string HexString(const T &value, bool is_spaced,  bool end_space, bool little_endian) {
        ...
        //TODO: why is this variable size? we always write 2 characters to it before adding it to the stream?
        //char str_buf[3];
        char str_buf[3]; //const 3 byte size (2 hex chars + null terminator)

        uint8_t *dat = (uint8_t*) &value; // c-style cast value as byte pointer
        std::stringstream stream; // declare a stringstream to store the hex octets
